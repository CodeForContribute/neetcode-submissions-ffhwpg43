...
class Solution {
    ...
    private boolean bfs(TreeNode root, long lower, long upper){
        if(root == null)return true;
        Queue<Object[]>q = new LinkedList();
        q.offer(new Object[]{root, lower, upper});
        while(!q.isEmpty()){
            int size = q.size();
            for(int i = 0; i < size;i++){
                ...
                if(node.left != null)q.offer(new Object[]{node.left, lower, node.val});
                if(node.right != null)q.offer(new Object[]{node.right, node.val, upper});
            }
        }
        return true;
    }
